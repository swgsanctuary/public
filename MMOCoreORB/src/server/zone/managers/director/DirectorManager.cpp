...
void DirectorManager::initializeLuaEngine(Lua* luaEngine) {
	...
	lua_register(luaEngine->getLuaState(), "getGalaxyName", getGalaxyName);
	lua_register(luaEngine->getLuaState(), "printLuaLog", printLuaLog);
...
int DirectorManager::broadcastGalaxy(lua_State* L) {
    if (checkArgumentCount(L, 2) == 1) {
        String err = "incorrect number of arguments passed to DirectorManager::broadcastGalaxy";
        printTraceError(L, err);
        ERROR_CODE = INCORRECT_ARGUMENTS;
        return 0;
    }
    ...
    if (faction != "" && faction != "all" && message != "") {
        chatManager->broadcastGalaxy(message, faction);
	}
	
	if (faction == "all" && message != "") {
...
int DirectorManager::printLuaLog(lua_State* L) {  //Prints to core3.log and lua.log without all the traceback nonsense
	if (checkArgumentCount(L, 1) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::printLuaLog";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}

	String logMessage = lua_tostring(L, -1);

	printErrorLog(L, logMessage);

	return 0;
}

void DirectorManager::printErrorLog(lua_State* L, const String& logMessage) {
	Lua* lua = instance()->getLuaInstance();
	//logMessage = lua_tostring(L, -1);
	lua->error(logMessage);
}
