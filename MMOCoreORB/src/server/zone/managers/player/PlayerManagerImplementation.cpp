...
void PlayerManagerImplementation::killPlayer(TangibleObject* attacker, CreatureObject* player, int typeofdeath, bool isCombatAction) {
	...
	if (attacker->isPlayerCreature() && attacker != player) {
		...
		box->setPromptText("Place a bounty on your killer! Enter an amount between 25,000 and 2,500,000 credits. The Bounty Hunter Guild will take 20% for its fee and your target will be added to our boards immediately.");
...
int PlayerManagerImplementation::calculateAttackers(CreatureObject* victim, CreatureObject* killer, ThreatMap* threatMap, int& groupSize){

	int count = 1;
	int groupId = 0;
	int grpSize = 0;

	//add everyone on the victim's threat list to the list, then iterate over the killer's group members that in range and count them as well making sure not to duplicate
	if (killer->isGrouped())
	...
	for (int i = 0; i < threatMap->size(); ++i) { //we should have been passed the victim's threatmap here as initial arguments for the function
		...
		if (!allowSameAccountPvpRatingCredit && defenderGhost->getAccountID() == attackerGhost->getAccountID()){
			//error("FRS: calculateAttackers ThreatMap: Participant is another character on the same account, disallowing FRS Credit");
			continue;
		}

		if (isSameFaction(victim, attacker)){
			//error("FRS: calculateAttackers ThreatMap: Participant is same faction, skipping");
			continue; //Cant be same faction
		}

		//error("FRS: calculateAttackers ThreatMap: Participant added to threatmap total.");
	...
	for (int i = 0; i < group->getGroupSize(); i++) {
		...
		if (member->getFactionStatus() != FactionStatus::OVERT && !memberGhost->hasBhTef()){
			//error("FRS: calculateAttackers GroupCheck: Participant is not overt or a BH with an active mission, not counting.");
		...
		/*if (!member->checkCooldownRecovery("rikers_timeout"))  {
			Time* cooldownTime = member->getCooldownTime("rikers_timeout");
			if (cooldownTime != nullptr) {
				float timeLeft = round(fabs(cooldownTime->miliDifference() / 100.f)) / 10.f;
				error("Rikers timeout remaining = " + String::valueOf(timeLeft) + " seconds.");
			}
		}*/
		...
		if (CombatManager::instance()->areInDuel(member, victim)){
			//error("FRS: calculateAttackers member and victim are in a duel, no FRS granted");
			continue;
		}

		if (isSameFaction(victim, member)){
			//error("FRS: calculateAttackers GroupCheck: Participant is same faction, skipping");
			continue; //Cant be same faction
		}
...
void PlayerManagerImplementation::awardPvpProgress(TangibleObject* killer, CreatureObject* destructedObject, bool performedDeathBlow = false) {
	...
	int groupSize = 0;
	int countDefenders = calculateAttackers(killerCreature,destructedObject,victimThreatMap,groupSize);
	//error("FRS: Involved Attackers for the death of: " + destructedObject->getFirstName());
	groupSize = -1;
	int countAttackers = calculateAttackers(destructedObject,killerCreature,attackerThreatMap,groupSize);
	//error("FRS:  GroupSize from calculateAttackers = " + String::valueOf(groupSize));

	error("FRS: New FRS Kill of " + destructedObject->getFirstName() + " [" + String::valueOf(countAttackers) + "] Attackers, [" + String::valueOf(countDefenders) + "] Defenders");
	...
	//error("FRS:  Award: ratio of the difference in group size calculated to: " + String::valueOf(sizeRatio));
	...
	//error("FRS: Award: modfiiedAward = " + String::valueOf(modifiedAward));
	...
	if (group != nullptr){//apply final penalty of up to 50% based on group size
		//error("FRS: Killer is grouped, adjusting down for group members, maximum penalty of 50%");

		int gSize;
		if (groupSize != -1)
			gSize = Math::min(10,groupSize);  // use the groupSize from calculate attackers from 0 to 10
		else
			gSize = Math::min(10,group->getGroupSize());// get the group size, cap it at 10

		//error("FRS: Award: groupSize value = " + String::valueOf(gSize));

		float groupPenalty = gSize * 0.05;
		...
		if (finalAward < (modifiedAward * 0.50)) {
			error("FRS: Final payout calculated to less than 50% of initial value, increasing to 50% payout) New Value = " + String::valueOf(finalAward));
			finalAward = modifiedAward * 0.50; //group size penalty will never be more than 50% of expected
		}
	}

	if (sizeRatio < 1){ //Killer won against overwhelming odds, they are granted a bonus of 25% for each additional attacker
		//error("FRS: Victim Won against a larger attacking force, bonus applied.");
		finalAward += ((modifiedAward * .25)/sizeRatio);
		//error("FRS: Award: +OverWhelming Bonus" + String::valueOf(finalAward));
	}
	...
	if ((killerCreature->hasSkill("force_rank_light_novice") || killerCreature->hasSkill("force_rank_dark_novice"))){
		...
		if (killedGhost->hasBhTef() && !destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
			//error("FRS: <Victim has recently, or is actively being hunted by a BH, no FRS is awarded>");
			...
			error("FRS: Award: Paying FRS to Killer, final Award:" + String::valueOf(finalAward));
	...
	if (group != nullptr) {
		...
		for (int i = 0; i < group->getGroupSize(); i++) {
			...
			if (!rikers && member->getZone() != destructedObject->getZone()){
				//error("FRS: " + member->getFirstName() + " is not in the same zone as the victim, skipping");
				continue;
			}
			...
			//error("FRS: Award: Start Check for Participant [" + member->getFirstName() +  "]");
			...
			if (!killedGhost->hasBhTef() || destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
				if (member->hasSkill("force_rank_light_novice") || member->hasSkill("force_rank_dark_novice")){
					...
					error("FRS: Award: Granting FRS to Participant [" + String::valueOf(finalAward) + "]");
	...
	if (sizeRatio >= 4){
		deductionXp = 0;
        error("FRS: Award: Overhwelming odds protection activated, no FRS XP Lost");
	} 
	...
	error("PVP Death: Zone = " + playerZone + ". Killer " + killerName + " coordinates (x=" + kX + ", y=" + kY + ", z=" + kZ + ").  Victim " + playerName + " coordinates (x=" + pX + ", y=" + pY + ", z=" + pZ + ").");
