...
bool PetManagerImplementation::handleCommandTraining(CreatureObject* speaker, AiAgent* pet, const String& message){
	...
	if (petType == CREATUREPET) {
		...
		if (!alreadyTrained) {
			...
			int skill = Math::max(speaker->getSkillMod("tame_level"),20);
			int level = petCreature->getAdultLevel();
			int roll = System::random(skill + level/5);

			SkillManager* skillManager = SkillManager::instance();
			int chBoxes = 0;
			if (skillManager != nullptr)
				chBoxes = skillManager->getSpecificSkillCount(speaker,"outdoors_creaturehandler");
			else
				error("Skillmanager is nullptr");
			
			if ((chBoxes/2 + skill) > roll)    // Better chance of success with more boxes in CH
				success = true;
