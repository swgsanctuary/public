...
void SkillManager::awardForceFromSkills(CreatureObject* creature) {
	...
	*/
	...
	if (force_max_diff != 0) {
		...
		error("Adjusting Force Power Max for player " + playerName + " by " + String::valueOf(-1*force_max_diff));
	}
	if (force_regen_diff !=0) {
		creature->addSkillMod(SkillModManager::PERMANENTMOD, "jedi_force_power_regen", -1*force_regen_diff, true);
		error("Adjusting Force Power Regen for player " + playerName + " by " + String::valueOf(-1*force_regen_diff));
	}
