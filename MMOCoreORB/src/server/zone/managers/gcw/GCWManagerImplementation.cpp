...
void GCWManagerImplementation::registerGCWBase(BuildingObject* building, bool initializeBase) {
	if (!hasBase(building)) {
		...
		if (initializeBase) {
			...
			if (delay == 0) {
				//error("registerGCWBase: Calling InitializeNewVulnerability");
			...
			if (delay == 0) {
				...
				addBase(building);
				//error("registerGCWBase: Calling StartVulnernability");
			...
			addBase(building);
			//error("registerGCWBase: Calling checkVulnerability");
			checkVulnerabilityData(building);
		}
	} else {
...
void GCWManagerImplementation::initializeBaseTimers(BuildingObject* building) {
	...
	if (getInitialVulnerabilityDelay() == 0) {
		Time nextVuln(baseData->getPlacementTime());
		nextVuln.addMiliTime(vulnerabilityFrequency * 1000);
		baseData->setNextVulnerableTime(nextVuln);
		//error("InitializeBaseTimers:  Next vulnerable time is " + nextVuln.getFormattedTime());
	} else {
		Time nextVuln(baseData->getPlacementTime());
...
void GCWManagerImplementation::startVulnerability(BuildingObject* building) {
	...
	verifyTurrets(building);
	//error("startVulnerability:   Calling scheduleVulnerabilityEnd.");
...
void GCWManagerImplementation::endVulnerability(BuildingObject* building) {
	...
	//error("endVulnerability:  next time = " + nextTime.getFormattedTime());
	...
	//error("endVulnerability:  new next time = " + nextTime.getFormattedTime());
...
void GCWManagerImplementation::initializeNewVulnerability(BuildingObject* building) {
	...
	//error("initializeNewVulnerability:  calling initialize new vulnerability.");
	initializeNewVulnerability(baseData);
}
...
void GCWManagerImplementation::scheduleVulnerabilityStart(BuildingObject* building) {
	...
	//error("scheduleVulnerabilityStart:  Scheduled vulnerability start time is " + vulnTime.getFormattedTime());

	if (vulnDif >= 0)
		return;

	//error("scheduleVulnerabilityStart:  Scheduling StartVulnerabilityTAsk");
...
void GCWManagerImplementation::scheduleVulnerabilityEnd(BuildingObject* building) {
	...
	//error("scheduleVulnerabilityEnd:  Scheduled vulnerability end time is " + endTime.getFormattedTime());

	if (endDif >= 0)
		return;
	
	//error("scheduleVulnerabilityEnd:  Scheduling new EndVulnerabilityTask to run in " + getDeltaTimeString(llabs(endDif)/1000));
...
void GCWManagerImplementation::refreshExpiredVulnerability(BuildingObject* building) {
	...
	while ((thisStartTime.getTime() + vulnerabilityFrequency) <= time(0)) {
	...
	if (!testTime.isPast()) {
		...
		//info("RefreshedExpiredVulnerability:  testTime is past", true);
		baseData->setLastVulnerableTime(thisStartTime);
		Time nStartTime(thisStartTime);
		nStartTime.addMiliTime(vulnerabilityFrequency * 1000);
		baseData->setNextVulnerableTime(nStartTime);
...
void GCWManagerImplementation::checkVulnerabilityData(BuildingObject* building) {
	...
	/*
	error("checkVulnerabilityData:  Current time = " + Time().getFormattedTime());
	error("checkVulnerabilityData:  Scheduled vulnerability start time is " + vulnTime.getFormattedTime());
	error("checkVulnerabilityData:  Scheduled vulnerability next end time is " + nextEnd.getFormattedTime());
	*/

	bool refreshed = false;

	if (!vulnTime.isPast()) {
		//error("vulnTime is not past");
		scheduleVulnerabilityStart(building);
	} else if (vulnTime.isPast() && !nextEnd.isPast()) {
		//error("vulnTime is past and nextEnd is not past.");
		startVulnerability(building);
	} else if (nextEnd.isPast()) {
		//error("nextEnd is  past.  Refreshing.");
		refreshed = true;
		refreshExpiredVulnerability(building);
	}
	if (!refreshed && nextEnd.isPast() && isBaseVulnerable(building) && building->getPvpStatusBitmask()) {
		//error("Found vulnerable PVP base with vulnerability end time in the past.  Refreshing.");
		refreshExpiredVulnerability(building);
	}
...
String GCWManagerImplementation::getVulnerableStatus(BuildingObject* building, CreatureObject* creature) {
	...
	return "@player_structure:next_vulnerability_prompt " + getDeltaTimeString(dif);
}
...
void GCWManagerImplementation::resetVulnerability(CreatureObject* creature, BuildingObject* building) {
	...
	if (ghost != nullptr) {
		...
		else if (isBaseVulnerable(building)) {   //Admins only
			creature->sendSystemMessage("Forcing base to non-vulnerable status and resetting timers.");
	...
	//error("Resetting vulnerability timer");
...
void GCWManagerImplementation::listBases(CreatureObject* creature) {

	Locker locker(_this.getReferenceUnsafeStaticCast());

	if (gcwBaseList.size() == 0) {
			creature->sendSystemMessage("No bases found on planet.");
			return;
	}
	...
	body << "Faction Bases on " << zoneName;
	...
	for (int i = 0; i < gcwBaseList.size(); i++) {
		...
		ManagedReference<CreatureObject*> owner = building->getOwnerCreatureObject();
		if (owner != nullptr)
			ownerName = building->getOwnerCreatureObject()->getFirstName();
		else
			continue;
		...
		body << "\tFaction:\t\t\t" << factionName << endl;

		if (building->getCustomObjectName().isEmpty())
			baseName = building->getDisplayedName();
		else
			baseName = building->getCustomObjectName().toString();

		templateName = building->getObjectTemplate()->getTemplateFileName();

		body << "\t\tName:\t\t\t" << baseName << endl;
		body << "\t\tType:\t\t\t" << templateName << endl;
		body << "\t\tOwner\t\t\t" << ownerName << endl;
		body << "\t\tLocation:\t\t" << ceil(building->getWorldPositionX()) << ", " << ceil(building->getWorldPositionY()) << endl;
		...
		String vulnerableStatus;
		String inString = ":";
		Time vulnerabilityEndTime = baseData->getVulnerabilityEndTime();
		Time nextVulnerableTime = baseData->getNextVulnerableTime();

		bool vulnerableNow = isBaseVulnerable(building);

		if (vulnerableNow) {
			vulnerableStatus = ":\t\tNow";
			deltaSeconds = vulnerabilityEndTime.getTime() - time(0);

		} else {
			deltaSeconds = nextVulnerableTime.getTime() - time(0); 
			vulnerableStatus = " In:\t" + getDeltaTimeString(deltaSeconds);
		}

		body << "\t\tVulnerable" << vulnerableStatus << endl;

		if (vulnerableNow && building->getPvpStatusBitmask()) {
			String deltaString = getDeltaTimeString(deltaSeconds);
			/*
			if (deltaString == "Error - Timer past due") {
				error("Base stuck vulnerable:   Current time = " + Time().getFormattedTime());
				error("Base stuck vulnerable:   Vulnerability end time = " + vulnerabilityEndTime.getFormattedTime());
				error("Base stuck vulnerable:   Next vulnerable time = " + nextVulnerableTime.getFormattedTime());
				error("Delta seconds = " + String::valueOf(deltaSeconds));
				error("Base stuck vulnerable:   Time overdue = " + getDeltaTimeString(llabs(deltaSeconds)));
			}
			*/
			body << "\t\tTime Left:\t\t" << getDeltaTimeString(deltaSeconds) << endl;
		}
	}
	...
	box->setPromptTitle("Faction Base List - " + zoneName);
	...
	ChatManager* chatManager = creature->getZoneServer()->getChatManager();
		if (chatManager != nullptr)
			chatManager->sendMail("System", "Faction Base List - " + zoneName, body.toString(), creature->getFirstName());

}

String GCWManagerImplementation::getDeltaTimeString(int64 durationSeconds) { //Time in seconds

	int64 seconds = durationSeconds; 
	
	if (seconds < 0)
		return "Error - Timer past due";
	...
	if (days > 0) {
		if (days == 1)
			dayString = String::valueOf(days) + " day, ";
		else
			dayString = String::valueOf(days) + " days, ";
		if (hours == 0 && minutes == 0)
			dayString = dayString.subString(0, dayString.length() - 2);
	}
	if (hours > 0) {
		if (hours == 1)
			hourString = String::valueOf(hours) + " hour, ";
		else
			hourString = String::valueOf(hours) + " hours, ";
		if (minutes == 0)
			hourString = hourString.subString(0, hourString.length() - 2);
	}
	if (minutes > 0) {
		if (minutes == 1)
			minuteString = String::valueOf(minutes) + " minute, ";

		else
			minuteString = String::valueOf(minutes) + " minutes, ";
	if ((seconds == 0) | (days != 0) | (hours != 0))
			minuteString = minuteString.subString(0, minuteString.length() - 2);
	}
	if (seconds > 0 && days == 0 && hours == 0) {
		if (seconds == 1)
			secondString = String::valueOf(seconds) + " second";
		else
			secondString = String::valueOf(seconds) + " seconds";
	}
