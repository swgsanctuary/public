...
bool MissionManagerImplementation::isBountyValidForPlayer(CreatureObject* player, PlayerBounty* bounty) {
	...
	int maxNumberOfMissions = 1; // 1 BH mission from Padawn to FRS rank 5

	/* if (creature->hasSkill("force_rank_dark_novice") || creature->hasSkill("force_rank_light_novice")) //1 hunters at FRS rank 0 - 5
		maxNumberOfMissions++; */

	if (creature->hasSkill("force_rank_dark_rank_06") || creature->hasSkill("force_rank_light_rank_06")) //2 hunters from rank 6 
		maxNumberOfMissions++;

	if (creature->hasSkill("force_rank_dark_rank_10") || creature->hasSkill("force_rank_light_rank_10")) //3 hunters for rank 10 and 11
		maxNumberOfMissions++;
	...
	if (zone != nullptr) {
		...
		if (planetManager != nullptr) {
			...
			if (terrainManager != nullptr) {
				...
				//error("Target Z = " + String::valueOf(targetZ) + ", Terrain Height = " + String::valueOf(height) + ", Difference = " + String::valueOf(heightDifference));

				if ((heightDifference) > 3.0f && building == nullptr) { // Over 3 meters above terrain and not in building, then check nearby objects for structure
	...
	for (int j = 0; j < hunters->size(); j++) {
		...
		if (hunter != nullptr) {
			...
			if (hunterGhost != nullptr && hunterGhost->getAccountID() == accountId)    // Can't have two characters on same account with same bounty
				return false;  
		}
	}
...
bool MissionManagerImplementation::sendPlayerBountyDebug(CreatureObject* creature, CreatureObject* target) {
	...
	if (playerBounty == nullptr) {
		...
		// String onlineStatus = targetGhost->isOnline() ? "True" : "False";
