...
float CombatManager::getDefenderJediToughnessModifier(CreatureObject* defender, int attackType, int damType, float damage) {
	...
	int forceDefense = 0;
	if (defender->isPlayerCreature() && defender->getPlayerObject()->isJedi())
		forceDefense = Math::min(defender->getSkillMod("force_defense"),125);;
...
int CombatManager::getArmorReduction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, float damage, int hitLocation, const CreatureAttackData& data) {
	...
	if (!data.isForceAttack()) {
		...
		if (absorbSkill > 0 && defender->isPlayerCreature() && !defender->hasBuff(BuffCRC::JEDI_FORCEBREACH) && defender->getSkillMod("avoid_incapacitation") < 1 && !wearingArmor) { //No absorb while using AI
			float damageToAbsorb = rawDamage;
			if (!attacker->isPlayerCreature())
				damageToAbsorb *= 0.6f; // nerf absorbed force vs NPC by 40%
...
bool CombatManager::isLightningAttack(const CreatureAttackData& data){
	if (data.getCombatSpam().beginsWith("forcelightning"))
		return true;
	else
		return false;
...
{
	if(data.getCombatSpam().beginsWith("flame"))
		return true;
	else
		return false;
}
...
float CombatManager::calculateDamage(CreatureObject* attacker, WeaponObject* weapon, CreatureObject* defender, const CreatureAttackData& data) {
	...
	if (attacker->hasSkill("combat_bountyhunter_novice")){
		if (attackerGhost != nullptr && defenderGhost != nullptr && defenderJediWeapon) { //mark of mandalore only works vs jedi
    ...
    if (!isDelayed && defenderGhost != nullptr && defender->hasBuff(BuffCRC::JEDI_FORTIFY) && !attacker->isIntimidated() &&
		...
		bool pvp = attacker->isPlayerCreature() || attacker->isPet();
		int forceCost = pvp ? 12 : 8;  // 12 in PVP, 8 in PVE
		...
		if (mdefForcePower >= forceCost) {  // Check for enough force first
			...
			if (roll <= intimidateAccuracy - intimidateDefense) {
				...
				data.getCommand()->applyEffect(defender, attacker, CommandEffect::INTIMIDATE, 0, 10.0); //This should apply the intimidate for 10 seconds
	...
	/*if (weapon->isSpecialHeavyWeapon()){
		if (!attacker->isPlayerCreature() && defender->isPlayerCreature())
			damage *= .75; // reduce npc special heavy weapon damage vs players by 25%
	}*/
	...
	if (data.isForceAttack()) {
		...
		//Force Lightning and Throw for player lightning and NPC's
		if (isLightningAttack(data) || data.getCombatSpam().contains("forcethrow")) {
			if (attacker->isPlayerCreature() && (defender->isPlayerCreature() || defender->isPet())) {
						damage *= 0.50;  // Reduce PVP lightning / Throw by 50%
			}
			if (!attacker->isPlayerCreature())
				damage *= .20; //80% damage reduction for NPCs using lightning abilities (after lightning "multiplier" moved to lua)
		}

		if (!attacker->isPlayerCreature() && (data.getCombatSpam() == "mindblast2" || data.getCombatSpam() == "mindblast1" || data.getCombatSpam() == "forcethrow2"
			|| data.getCombatSpam() == "forcethrow1"))
				damage *= .30;

		if (defender->isPlayerCreature() && attacker->isPlayerCreature() && data.getCombatSpam() == "mindblast2") //pvp reduction for mindblast2
			damage *= .85;
...
void CombatManager::applyStates(CreatureObject* creature, CreatureObject* targetCreature, const CreatureAttackData& data) {
	...
	for (int i = 0; i < stateEffects->size(); i++) {
		...
		if (!failed && targetCreature->isPlayerCreature() && targetCreature->getPlayerObject()->isJedi()) {
			...
			if (creature->isPlayerCreature()){   // Only cap if attacker is player
				if (targetDefense > 88)
			...
			if (creatureGhost != nullptr && targetGhost != nullptr)
				if (creatureGhost->hasBhTef() && targetGhost->hasBhTef()) {
					targetDefense *= .9;
