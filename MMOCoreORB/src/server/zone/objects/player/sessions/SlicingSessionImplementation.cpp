...
//			suiBox->addMenuItem("Random slice.", 8);    // ** Removed 
...
void SlicingSessionImplementation::handleWeaponSlice() {
	...
	if(!selectedSlice){
		switch(System::random(1)) {
			...
			// percentage = ceil(percentage * 0.85);   // 15% penalty for choosing type of slice  ** Removed **
			switch(sliceOption) {
				case 2:
					handleSliceDamage(percentage);
					break;
...
void SlicingSessionImplementation::handleArmorSlice() {
	...
	if(!selectedSlice){
		...
		// percent = ceil(percent * 0.85);   // 15% penalty for choosing type of slice  ** Removed **
		switch (sliceOption) {
			case 1:
				handleSliceEffectiveness(percent);
				break;
...
void SlicingSessionImplementation::broadcastResult(StringIdChatParameter& stringid) {
	...
	thisZone->getInRangeObjects(player->getWorldPositionX(), player->getWorldPositionY(), 7.5, closeObjects, true);
	...
	for (int i = 0; i < closeObjects->size(); ++i) {
		...
		CreatureObject* otherPlayer = object->asCreatureObject();
