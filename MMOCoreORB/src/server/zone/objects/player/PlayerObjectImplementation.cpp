...
void PlayerObjectImplementation::notifySceneReady() {
	...
	if (zone != nullptr  && creature->getFirstName().toLowerCase() != "discord") {  //Not Discord Bot
	...
	for (int i = chatRooms.size() - 1; i >= 0; i--) {
		...
		if (room != nullptr && creature->getFirstName().toLowerCase() != "discord") {    //Not Discord Bot
	...
	//Let's make Discord Chat Bot Join the Infinity Channel
	if (creature->getFirstName().toLowerCase() == "discord") {
...
void PlayerObjectImplementation::notifyOnline() {
	...
	// Infinity dev admin access for TC
	if (zoneServer->getGalaxyName() == "SWG Infinity TC" && (firstName == "bbq" || firstName == "mannax-" || firstName == "stepo")) {

		PlayerManager* playerManager = server->getPlayerManager();
		SkillManager* skillManager = zoneServer->getSkillManager();

		if (playerManager != nullptr && skillManager != nullptr) {
			skillManager->addAbility(_this.getReferenceUnsafeStaticCast(), "admin");
			playerManager->updatePermissionLevel(playerCreature, 15);
		}
	}
	...
	*luaOnPlayerLoggedIn << playerCreature;
	...
	if (isJedi()) {
		...
		setScreenPlayData("frs", "login_timestamp", String::valueOf(currentTimeMili));
	}
	...
	// Bump bank up to 500 items
	SceneObject* bank = playerCreature->getSlottedObject("bank");

	if (bank != nullptr) {
		int maxBankItemLimit = 500;
		int currentItemLimit = bank->getContainerVolumeLimit();
		if (currentItemLimit < 500)
			bank->setContainerVolumeLimit(maxBankItemLimit);
	}

	//Let's unequip uncertified and zero condition armor and clothing

	bool itemsRemain = true;
	int attempts = 0;
	int numObjects;

	while (itemsRemain && (attempts <= 5)) {

		itemsRemain = false;
		attempts += 1;

		//error("Checking items pass #" + String::valueOf(attempts));
		//error("There are " + String::valueOf(playerCreature->getSlottedObjectsSize()) + " slotted items.");

		for (int i = 0; i < playerCreature->getSlottedObjectsSize(); ++i) {
				
			ManagedReference<TangibleObject*> object = playerCreature->getSlottedObject(i).castTo<TangibleObject*>();

			if (object == nullptr)
				continue;					
			
			//error("Checking object " + object->getObjectName()->getStringID());

			if (object->isContainerObject()) {
				if (object->getObjectName()->getStringID().contains("backpack") && object->getContainerVolumeLimit() == 65) {
					int capacity = 50;
					Locker locker(object);
					object->setContainerVolumeLimit(capacity);
					locker.release();
					error("Setting container volume limit on faction backpack to " + String::valueOf(capacity) + " for player = " + playerCreature->getFirstName());
				}
				continue;
			}
			...
			if (object->isWearableObject()) {
				...
				if ((object->getMaxCondition() - object->getConditionDamage()) <= 0) {    // Let's remove wearables with zero condition
					...
					if (inventory != nullptr){ 
						...
						info("Removing zero condition wearable " + object->getObjectName()->getStringID() + " for player " + playerCreature->getFirstName(),true);
						continue;
					}
				}

				//error("Checking wearable object " + object->getObjectName()->getStringID());

				SharedTangibleObjectTemplate* tanoData = dynamic_cast<SharedTangibleObjectTemplate*>(object->getObjectTemplate());
				if (tanoData != nullptr) {
					const Vector<String>& skillsRequired = tanoData->getCertificationsRequired();

					if (skillsRequired.size() > 0) {   // We have a wearable item with certifications required
						bool hasSkill = false;

						for (int i = 0; i < skillsRequired.size(); i++) {
							const String& skill = skillsRequired.get(i);

							if (!skill.isEmpty() && playerCreature->hasSkill(skill)) {
								hasSkill = true;
								break;
							}
						}
						...
						SceneObject* inventory = playerCreature->getSlottedObject("inventory");
						if (inventory != nullptr && !hasSkill){ // Remove uncertified wearable item
							ZoneServer* zoneServer = server->getZoneServer();
							ObjectController* objectController = zoneServer->getObjectController();
							objectController->transferObject(object, inventory, -1, true, true); // -1 is the containmentType to unequip items
							info("Removing uncertified wearable " + object->getObjectName()->getStringID() + " for player " + playerCreature->getFirstName(),true);
							itemsRemain  = true;
						}
					}
				}
