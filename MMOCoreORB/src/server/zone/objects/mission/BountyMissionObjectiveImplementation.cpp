		...
		if (owner->getObjectID() == killer->getObjectID()) {
			...
			if (zoneServer != nullptr) {
				...
				if (target != nullptr) {
					...
					zBroadcast << "\\#66B3FF" << "[Bounty Hunter Guild Alert]" << "\\#FFFFFF" << " Guild member \\#FF0000" << hunterName << "\\#FFFFFF has claimed the Bounty on \\#FFD700" << targetName << "\\#FFFFFF.";
			...
			if (npcTarget == nullptr) {   //Only send messages for player missions
				...
				// Setup broadcast
				StringBuffer zBroadcast;
				zBroadcast << "\\#FFFFFF" << "[Intercepted Hutt Transmission] A marked target by the name of < \\#FFD700" << huntedName << "\\#FFFFFF > has defeated a Bounty Hunter named < \\#FF0000" << hunterName << "\\#FFFFFF >!";
