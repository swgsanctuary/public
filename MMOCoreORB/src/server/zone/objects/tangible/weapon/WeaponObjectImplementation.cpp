...
void WeaponObjectImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	...
	if (roundedForceCost < 1) roundedForceCost = 1;   // 1 Minimum force cost on sabers
...
void WeaponObjectImplementation::decay(CreatureObject* user) {
	...
	if (hasPowerup())
		chance += 3;    // Increase chance of damage by 60%
	...
	if (roll < chance) {
		...
		if (isJediWeapon()) {
			...
			// Roll again with a 25% chance to fail to reduce durability loss on LS components
