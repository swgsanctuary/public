...
void PetMenuComponent::fillObjectMenuResponse(SceneObject* sceneObject, ObjectMenuResponse* menuResponse, CreatureObject* player) const {
	...
	if (pet->isIncapacitated() && owner != nullptr && owner->isAttackableBy(player))
	...
	// CREATURES
		...
		if( player->hasSkill( "outdoors_creaturehandler_master" ) ){
			...
			bool varactylBelsavis = false;
			ManagedReference<Creature*> petCreature = cast<Creature*>(pet);
			if (petCreature != nullptr) {
				String templateString = petCreature->getControlDeviceTemplate();
				if (templateString != "")
					if (templateString.contains("belsavis_varactyl"))
						varactylBelsavis = true;
			}
			if (!varactylBelsavis)
