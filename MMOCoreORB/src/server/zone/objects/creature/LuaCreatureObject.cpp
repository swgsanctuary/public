...
int LuaCreatureObject::addDotState(lua_State* L) {
	//addDotState(unsigned long dotType, unsigned int strength, byte type, unsigned int duration, float potency, unsigned int defense, int secondaryStrength)

	
	CreatureObject* attacker = nullptr;
	uint32 defense, objectID, duration, strength;
	float potency;
	byte type;
	uint64 dotType;
	...
	if (numberOfArguments == 9) {
		attacker = (CreatureObject*)lua_touserdata(L, -9);
		dotType = lua_tointeger(L, -8);
		strength = lua_tointeger(L, -7);
		type = lua_tointeger(L, -6);
		duration = lua_tointeger(L, -5);
		potency = lua_tonumber(L, -4);
		objectID = lua_tointeger(L, -3);
		defense = lua_tointeger(L, -2);
		secondaryStrength = lua_tointeger(L, -1);
	}
	else {
		attacker = (CreatureObject*)lua_touserdata(L, -8);
		dotType = lua_tointeger(L, -7);
		strength = lua_tointeger(L, -6);
		type = lua_tointeger(L, -5);
		duration = lua_tointeger(L, -4);
		potency = lua_tonumber(L, -3);
		objectID = lua_tointeger(L, -2);
		defense = lua_tointeger(L, -1);
	}

	Locker locker(realObject);
	realObject->addDotState(attacker, dotType, objectID, strength, type, duration, potency, defense, secondaryStrength);
...
int LuaCreatureObject::enhanceCreature(lua_State* L) {
	...
	if (numberOfArguments == 4) {
		...
		Logger::console.error("LuaCreatureObject::enhanceCreature incorrect number of arguments (4 required).");
