...
int CreatureObjectImplementation::inflictDamage(TangibleObject* attacker, int damageType, float damage, bool destroy, bool notifyClient, bool isCombatAction) {
	...
	if (getSkillMod("avoid_incapacitation") > 0) {
		damage *= 0.20;    							//80% damage reduction after all other reductionsn));
	}
...
bool CreatureObjectImplementation::isHealableBy(CreatureObject* healer) {
	
	CreatureObject* targetCreo = asCreatureObject();

	if (healer == targetCreo)	// Can always heal yourself
		return true;

	if (isInvisible())
		return false;

	if (isAttackableBy(healer))
		return false;

	PlayerObject* healerGhost = healer->getPlayerObject(); // playerObject of healer
	...
	if (isPet()) {
		...
		if (linkedCreature != nullptr) {
			...
			if (healer == targetCreo)	// Can always heal your pets
				return true;
		}
	}

	if (healerGhost->hasBhTef() && !healerGhost->hasPvpTef()) {
		return false; //BH tef prevents healing unless the healer is also PVP Teffed
	}

	uint32 targetFactionStatus = targetCreo->getFactionStatus();
	uint32 currentFactionStatus = healer->getFactionStatus();

	if (getFaction() != healer->getFaction() && !(targetFactionStatus == FactionStatus::ONLEAVE))
		return false;
