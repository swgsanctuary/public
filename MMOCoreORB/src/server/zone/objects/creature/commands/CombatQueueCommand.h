...
public:
	...
	virtual void applyEffect(CreatureObject* attacker, CreatureObject* defender, uint64 effectType, uint32 mod, float defaultDuration = 5.0) const {
		CombatManager* combatManager = CombatManager::instance();
		...
		//attacker->sendSystemMessage("TC:  ApplyState Intimidate statelength = " + String::valueOf(effect.getStateLength()));
		//attacker->sendSystemMessage("TC:  ApplyState Intimidate defense before mod = " + String::valueOf(targetDefense));

		targetDefense -= mod;

		//attacker->sendSystemMessage("TC:  ApplyState Intimidate defense after mod = " + String::valueOf(targetDefense));

		uint32 duration = (uint32) Math::max(defaultDuration, effect.getStateLength()*(1.f-targetDefense/120.f));

		//attacker->sendSystemMessage("TC:  ApplyState Duration = " + String::valueOf(duration));
