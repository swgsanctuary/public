...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		for (int i = 0; i < closeObjects.size(); i++) {
			...
			if (object != nullptr && object->getGameObjectType() == SceneObjectType::TRAVELTERMINAL && checkDistance(creature, object, 30)) {
				nearTravelTerminal = true;
				break;
			}
		}

		if (nearTravelTerminal) {   //Make sure they are not on top of a structure
			...
			if (zone != nullptr) {
				...
				if (planetManager != nullptr) {
					...
					if (terrainManager != nullptr) {
						...
						if (heightDifference > 5.0) {
							creature->sendSystemMessage("You are too far above the ground to purchase a ticket.");
							return GENERALERROR;
						}
					}
				}
		...
		if (currentCity != nullptr) { 
			bool inPlayerCity = !currentCity->isClientRegion();
   			if (creature->getParent() != nullptr && inPlayerCity) {
				creature->sendSystemMessage("You cannot purchase a ticket while inside player housing or player city structures.");
  				return GENERALERROR;
  			}  
		}
