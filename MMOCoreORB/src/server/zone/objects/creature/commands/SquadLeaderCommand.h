...
public:
	...
	static bool isValidGroupAbilityTarget(CreatureObject* leader, CreatureObject* target, bool allowPet) {
		...
		if(!target->isInRange(leader, 256)) // Skip this group member who is out of range (over 256 meters)
			return false; 
		...
		if (ghost->hasBhTef() && !(ghost->hasPvpTef() && leaderGhost->hasPvpTef())) //If the SL and the BH are both PVP Teffed then the buff will work
			return false;
