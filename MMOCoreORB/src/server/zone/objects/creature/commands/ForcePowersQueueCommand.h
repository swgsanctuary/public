...
public:
	...
	int doCombatAction(CreatureObject* creature, const uint64& target, const UnicodeString& arguments = "") const {
			...
			try {
				...
				if (playerObject != nullptr) {
					//creature->sendSystemMessage("*TC* Force powers attack reducing force by " + String::valueOf(forceCost));
	...
	float getCommandDuration(CreatureObject *object, const UnicodeString& arguments) const {
		...
		combatHaste = combatHaste/2; //combat haste was too effective for powers post rework
