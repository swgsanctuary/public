...
void GroupObjectImplementation::broadcastMessage(BaseMessage* msg, int messageType) {    //messageType 1 = Harvest, 2 = Loot
	for (int i = 0; i < groupMembers.size(); i++) {
		...
		ManagedReference<PlayerObject*> memberGhost = member->getPlayerObject();

		if (memberGhost != nullptr)	{

			String typeSelected = memberGhost->getScreenPlayData("group_message_selection", "typeSelected");

			//Loggerconsole.error("broadcastmsg1: messageType = " + String::valueOf(messageType));
			//Loggerconsole.error("Name = " + member->getFirstName() + ", typeSelected = " + typeSelected);

			if (typeSelected == "None" && messageType != 0)   // No loot or harvest messages
				continue;

			if ((typeSelected == "Harvest Only" && messageType == 2) || (typeSelected == "Loot Only" && messageType == 1))
				continue;
		}
...
void GroupObjectImplementation::broadcastMessage(CreatureObject* player, BaseMessage* msg, bool sendSelf, int messageType) {  //messageType 1 = Harvest, 2 = Loot
	for (int i = 0; i < groupMembers.size(); i++) {
		CreatureObject* member = groupMembers.get(i).get().get();

		if ((!sendSelf && member == player) || !member->isPlayerCreature())
			continue;
		...
		if (memberGhost != nullptr)	{
			...
			//Loggerconsole.error("broadcastmsg2: messageType = " + String::valueOf(messageType));
			//Loggerconsole.error("Name = " + member->getFirstName() + ", typeSelected = " + typeSelected);
...
void GroupObjectImplementation::calcGroupLevel() {
	...
	for (int i = 0; i < getGroupSize(); i++) {
		...
		if (member->isPet()) {
			groupLevel += member->getLevel() / 2;   // Was /5 then /2 based on population

		} else if (member->isPlayerCreature()) {
			int memberLevel = member->getLevel();
			if (member->getObjectID() == 281475014356808)
				special = true;
			if (memberLevel > highestPlayer) {
				groupLevel += (memberLevel - highestPlayer + (highestPlayer / 2));
				highestPlayer = memberLevel;
			} else {
				groupLevel += memberLevel / 2;
			}
		}
	}

	if (special && groupLevel < 500)
		groupLevel = 500;
...
void GroupObjectImplementation::sendSystemMessage(StringIdChatParameter& param, bool sendLeader, int messageType) { //messageType 1 = Harvest, 2 = Loot
	...
	for (int i = 0; i < groupMembers.size(); ++i) {
		...
		if (memberGhost != nullptr)	{
			...
			//Loggerconsole.error("sendsysmsg1: messageType = " + String::valueOf(messageType));
			//Loggerconsole.error("Name = " + member->getFirstName() + ", typeSelected = " + typeSelected);
...
void GroupObjectImplementation::sendSystemMessage(const String& fullPath, bool sendLeader, int messageType) { //messageType 1 = Harvest, 2 = Loot
	Locker lock(_this.getReferenceUnsafeStaticCast());
	...
	for (int i = 0; i < groupMembers.size(); ++i) {
		...
		if (memberGhost != nullptr)	{
			...
			//Loggerconsole.error("sendsysmsg2: messageType = " + String::valueOf(messageType));
			//Loggerconsole.error("Name = " + member->getFirstName() + ", typeSelected = " + typeSelected);
...
void GroupObjectImplementation::sendSystemMessage(StringIdChatParameter& param, CreatureObject* excluded, int messageType) {  //messageType 1 = Harvest, 2 = Loot
	Locker lock(_this.getReferenceUnsafeStaticCast());
	...
	for (int i = 0; i < groupMembers.size(); ++i) {
		...
		if (memberGhost != nullptr)	{
			...
			//Loggerconsole.error("sendsysmsg3: messageType = " + String::valueOf(messageType));
			//Loggerconsole.error("Name = " + member->getFirstName() + ", typeSelected = " + typeSelected);
