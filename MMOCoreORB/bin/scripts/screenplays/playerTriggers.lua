...
function PlayerTriggers:playerLoggedIn(pPlayer)
	...
	if (not jediKnight and (darkNovice or lightNovice)) or 
		...
		(jediState > 2 and not darkNovice and not lightNovice) then
		...
		PlayerObject(pGhost):setFrsRank(pGhost, -1)
		PlayerObject(pGhost):setFrsCouncil(pGhost, 0)
		...
		printLuaLog("PlayerTriggers:  Removed all FRS skills and XP for Jedi " .. CreatureObject(pPlayer):getFirstName())
