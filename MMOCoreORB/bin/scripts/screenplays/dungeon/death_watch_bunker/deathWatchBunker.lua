...
DeathWatchBunkerScreenPlay = ScreenPlay:new {
	...
	bunkerItems = {
		...
		mandoHelmet = "object/tangible/loot/loot_schematic/death_watch_mandalorian_helmet_schematic.iff",
...
function DeathWatchBunkerScreenPlay:hasPermission(pPlayer, permissionGroup)
	...
	print("DeathWatchBunkerScreenPlay:hasPermission(pPlayer, permissionGroup)")
	if util:isAdmin(pPlayer) then
		print("DeathWatchBunkerScreenPlay:hasPermission(pPlayer, permissionGroup) = true")
		return true
	end
...
function DeathWatchBunkerScreenPlay:onEnterDWB(sceneObject, pCreature)
	...
	if util:isAdmin(pCreature) then
		self:adminUnlockAll(pCreature)
	end
	return 0
end
...
function DeathWatchBunkerScreenPlay:adminUnlockAll(pCreature)
	CreatureObject(pCreature):setScreenPlayState(2, "death_watch_bunker")
	CreatureObject(pCreature):setScreenPlayState(4, "death_watch_bunker")
	CreatureObject(pCreature):setScreenPlayState(8, "death_watch_bunker")
	CreatureObject(pCreature):setScreenPlayState(16, "death_watch_bunker")
	CreatureObject(pCreature):setScreenPlayState(32, "death_watch_bunker")
	CreatureObject(pCreature):setScreenPlayState(64, "death_watch_bunker")
	CreatureObject(pCreature):setScreenPlayState(128, "death_watch_bunker")

	for i = 1, #self.doorData, 1 do
		self:givePermission(pCreature, "DeathWatchBunkerDoor" .. i)
	end

	CreatureObject(pCreature):sendSystemMessage("[MGMT] all doors unlocked")
end
